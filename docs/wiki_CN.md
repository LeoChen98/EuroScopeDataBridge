...
- **Request/Response（请求/响应）**：客户端发送带唯一 `id` 的请求，服务端在处理后返回带有相同 `id` 的响应。请求由 `OnTimer` 回调在 EuroScope 主线程排空处理——这是 EuroScope SDK 访问唯一安全的上下文。

定时行为：

- 入站请求由 EuroScope 的 `OnTimer` 回调每秒处理一次。
...
| 请求处理间隔 | `1 s` | `OnTimer` 在 EuroScope 主线程排空并处理请求 |
